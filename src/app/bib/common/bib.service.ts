...
export class BibService {
  ...
  //GET ALL BIB ENTRIES
  getBibEntries(): Observable<IBib[]> {
    //Gets an observable of type IBib[]. Can be accessed and subscribed to by other classes to access data.
    return this.httpClient.get<IBib[]>(`${this.bibMain}/getbibentries`).pipe(
      tap((data) => console.log("All bib data got: " + JSON.stringify(data))),
  ...
  getBibEntryById(id: number): Observable<IBib> {
    return this.httpClient
      .get<IBib>(`${this.bibMain}/getbibentrybyid/${id}`)
      .pipe(
        tap((data) =>
          console.log(`Bib entry with ID ${id}: ` + JSON.stringify(data)),
        ),
        catchError(this.errorHandler),
      );
  }
  ...
  postBib(bib: IBib): Observable<IBib> {
    ...
    return this.httpClient.post<IBib>(`${this.bibMain}/postbibentry`, bib).pipe(
      tap((data) => console.log("Bib entry posted: " + JSON.stringify(data))),
      catchError(this.errorHandler),
    );
  }
