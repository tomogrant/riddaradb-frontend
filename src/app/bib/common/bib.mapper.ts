...
export class BibMapper {
  mapDtoToVm(dto: IBib): IBibVm {
    var vm: IBibVm = {
      //ID can't be null at this point
      id: dto.id ?? 0,
  ...
  buildJournalArticle(dto: IBib): string {
    ...
    if (dto.authors !== "") {
      if (dto.editors === "" && dto.translators === "") {
        str = str + (dto.authors + ". ");
      } else {
        str = str + (dto.authors + ", ");
      }
  ...
  buildBookChapter(dto: IBib): string {
    ...
    if (dto.pageNumbers.includes("-")) {
      str = str + ("pp. " + dto.pageNumbers + ".");
    } else {
      str = str + ("p. " + dto.pageNumbers + ".");
    }
