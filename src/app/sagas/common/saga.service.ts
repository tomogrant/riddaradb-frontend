...
export class SagaService {
  ...
  postSaga(saga: ISagaRequestDto): Observable<ISagaResponseDto> {
    ...
    return this.httpClient.post<ISagaResponseDto>(`${this.sagasMain}/postsaga`, saga).pipe(
      tap((data) => console.log("Saga posted: " + JSON.stringify(data))),
      catchError(this.errorHandler),
    );
  }

  //READ ALL SAGA VMS
  getSagas(): Observable<ISagaResponseDto[]> {
    //Gets an observable of type ISaga[]. Can be accessed and subscribed to by other classes to access data.
    return this.httpClient.get<ISagaResponseDto[]>(`${this.sagasMain}/getsagas`).pipe(
      tap((data) => console.log("All saga data got: " + JSON.stringify(data))),
      catchError(this.errorHandler),
    );
  }
  ...
  getSagaById(id: number): Observable<ISagaResponseDto> {
    return this.httpClient.get<ISagaResponseDto>(`${this.sagasMain}/getsagabyid/${id}`).pipe(
      tap((data) => console.log(`Saga with ID ${id}: ` + JSON.stringify(data))),
      catchError(this.errorHandler),
    );
  }

  getSagaTitles(): Observable<ISagaTitleDto[]> {
    return this.httpClient.get<ISagaTitleDto[]>(`${this.sagasMain}/getsagatitles`).pipe(
      tap((data) => console.log("All saga title data got: " + JSON.stringify(data))),
      catchError(this.errorHandler),
    );
  }
  ...
  putSaga(saga: ISagaRequestDto): Observable<ISagaResponseDto> {
    ...
    return this.httpClient.put<ISagaResponseDto>(`${this.sagasMain}/putsaga`, saga).pipe(
