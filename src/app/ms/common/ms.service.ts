...
export class MsService {
  ...
  //GET ALL MS ENTRIES
  getMsEntries(): Observable<IMs[]> {
    //Gets an observable of type IMS[]. Can be accessed and subscribed to by other classes to access data.
    return this.httpClient.get<IMs[]>(`${this.msMain}/getmsentries`).pipe(
      tap((data) => console.log("All MS data got: " + JSON.stringify(data))),
      catchError(this.errorHandler),
    );
  }
  ...
  getMsEntryById(id: number): Observable<IMs> {
    return this.httpClient.get<IMs>(`${this.msMain}/getmsentrybyid/${id}`).pipe(
      tap((data) => console.log(`MS entry with ID ${id}: ` + JSON.stringify(data))),
      catchError(this.errorHandler),
    );
  }

  //GET ALL MS ENTRIES
  getMsEntriesByRepoId(id: number): Observable<IMs[]> {
    //Gets an observable of type IMS[]. Can be accessed and subscribed to by other classes to access data.
    return this.httpClient.get<IMs[]>(`${this.msMain}/getmsentriesbyrepoid/${id}`).pipe(
      tap((data) => console.log(`MS data from repo ${id}: ` + JSON.stringify(data))),
      catchError(this.errorHandler),
    );
  }
  ...
  postMs(ms: IMs): Observable<IMs> {
    ...
    return this.httpClient.post<IMs>(`${this.msMain}/postmsentry`, ms).pipe(
      tap((data) => console.log("MS entry posted: " + JSON.stringify(data))),
      catchError(this.errorHandler),
    );
  }
  ...
  putMs(ms: IMs): Observable<IMs> {
    ...
    return this.httpClient.put<IMs>(`${this.msMain}/putmsentry`, ms).pipe(
      tap((data) => console.log("MS updated: " + JSON.stringify(data))),
      catchError(this.errorHandler),
    );
  }
