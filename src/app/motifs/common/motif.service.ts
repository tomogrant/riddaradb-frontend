...
export class MotifService {
  ...
  postMotif(motif: IMotif): Observable<IMotif> {
    return this.httpClient.post<IMotif>(`${this.motifMain}/postmotif`, motif).pipe(
      tap((motif) => console.log("Motif posted: " + JSON.stringify(motif))),
  ...
  getRootMotifs(): Observable<IMotif[]> {
    return this.httpClient.get<IMotif[]>(`${this.motifMain}/getrootmotifs`).pipe(
      tap((motifs) => console.log("Root motifs returned: " + JSON.stringify(motifs))),
      catchError(this.errorHandler),
    );
  }
  ...
  getChildren(id: number): Observable<IMotif[]> {
    return this.httpClient.get<IMotif[]>(`${this.motifMain}/getchildmotifsbyid/${id}`).pipe(
      tap((motifs) => console.log("Child motifs of ID " + id + ": " + JSON.stringify(motifs))),
      catchError(this.errorHandler),
    );
  }
  ...
  searchMotifs(searchTerm: string): Observable<IMotifSearchResult[]> {
    return this.httpClient.get<IMotifSearchResult[]>(`${this.motifMain}/searchmotifs/${searchTerm}`).pipe(
      tap((results) => console.log("Search results: " + JSON.stringify(results))),
      catchError(this.errorHandler),
    );
  }
  ...
  searchMotifsExactMatch(searchTerm: string): Observable<IMotifSearchResult[]> {
    return this.httpClient.get<IMotifSearchResult[]>(`${this.motifMain}/searchmotifsexact/${searchTerm}`).pipe(
      tap((results) => console.log("Search results: " + JSON.stringify(results))),
      catchError(this.errorHandler),
    );
  }
